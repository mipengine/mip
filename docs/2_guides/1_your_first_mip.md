...
下面这段代码是一个基础的 MIP HTML 的 demo 示例。你可以复制到html文件中，并根据规范添加你想要的元素。需要注意的是`//m.baidu.com/static/ala/sf/static/css/miphtml_xxxxxx.css` 和 `//m.baidu.com/static/ala/sf/static/js/miphtml_main_xxxxxx.js` 是必须要加载的css和js静态文件，`xxxxxx`是版本号，版本号会根据功能的升级而不同，开发中可以根据文档中的说明进行加载。
